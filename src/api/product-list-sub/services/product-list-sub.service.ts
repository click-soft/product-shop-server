...
export class ProductListSubService {
  ...
  private async getProductListSubsByList(
    jisa: string,
    productLists: ProductList[],
  ): Promise<ProductListSub[]> {
    const smCodes = productLists.map((pl) => pl.smCode);
    return await this.productListSubRepository
      .createQueryBuilder()
  ...
  async getLatestProducts(): Promise<ProductListSub[]> {
    ...
    const lastestData = dataList.reduce(
      (acc: ProductListSub[], cur: ProductListSub) => {
  ...
  async getAll(filter: ProductArgs): Promise<ProductsByBunryu[] | undefined> {
    const productLists = await this.productListService.getProductLists(
      filter.jisa,
      filter.bunryu,
    );
    ...
    const plsResult = await this.getProductListSubsByList(
      filter.jisa,
      productLists,
    );

    const lastestPrds = plsResult.reduce(
      (acc: ProductsByBunryu[], cur: ProductListSub) => {
        ...
        if (!prd) {
          ...
          const findIndex = prd.products?.findIndex(
            (p) => p.smCode === cur.smCode,
          );
